...
- **Default profiles (read carefully).** New Layer flags should default **off / zero**, except documented exceptions. Today `ProcessFaults.fouling_dynamic` defaults to **`True`** (Layer 2.5 on → `sv` width 22). Bare `ProcessFaults()` is **not** the upstream legacy pin path.
  ...
  | Legacy fingerprint | `fouling_dynamic=False` (+ other masters off/zero) | batch `sv=6f61eb53…`, live `sv=f37fb5e0…` |
  | Layer 2.5 fingerprint | `fouling_dynamic=True`, extras off | batch `sv=1938fec8…` |
  | Layer 2.6 active | disturbance amplitudes on | batch `sv=e2a29849…`, live `sv=13ea81f3…` |
